    ...
    <Layout>
      <div className="flex flex-col gap-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
          <StatCard
            title="Active Agents"
            value={metrics?.activeAgents ?? agents.filter((a) => a.status === "running").length}
            icon={Zap}
            color="blue"
          />
          <StatCard
            title="Total Agents"
            value={metrics?.totalAgents ?? agents.length}
            icon={Bot}
            color="emerald"
          />
          <StatCard
            title="Queued Agents"
            value={metrics?.queue?.waiting ?? 0}
            icon={Cpu}
            color="purple"
          />
        </div>
        ...
        <div className="flex flex-col gap-6">
          <div className="flex items-center justify-between">
            ...
            <a href="/app/agents" className="text-sm font-bold text-blue-600 hover:underline">
              View All
            </a>
          </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-slate-100 bg-slate-50/50">
                      <th className="px-4 sm:px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Agent
                      </th>
                      <th className="px-4 sm:px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        Status
                      </th>
                      <th className="px-4 sm:px-8 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest hidden sm:table-cell">
                        Host
                      </th>
                  ...
                  <tbody>
                    {agents.slice(0, 5).map((agent) => (
                      ...
                      >
                          ...
                          <div className="flex items-center gap-1.5">
                            <div
                              className={clsx(
                                ...
                                agent.status === "running" ? "bg-emerald-500" : "bg-blue-500",
                              )}
                            ></div>
                        ...
                        <td className="px-4 sm:px-8 py-4 text-xs font-medium text-slate-500 hidden sm:table-cell">
                          {agent.node || "Local host"}
                        </td>
                        <td className="px-4 sm:px-8 py-4 text-right">
                          <a
                            href={`/app/agents/${agent.id}`}
                            className="inline-flex p-2 hover:bg-white rounded-lg border border-transparent hover:border-slate-200 transition-all"
                          >
