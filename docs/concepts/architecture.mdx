...
The public repo centers on a single control-plane host. Agent workloads can stay on the local Docker host or be placed onto GA Kubernetes targets without changing the operator workflow. NemoClaw is an experimental sandbox profile layered onto supported targets, and Proxmox is a planned execution target that remains release-blocked today.
...
flowchart LR
    ...
    subgraph RuntimeInfra["Runtime execution layer"]
        Provisioner --> Adapters["Backend adapters<br/>Docker / k3s/k8s / Proxmox (blocked)"]
...
| Deploy target   | `docker`, `k8s`, `proxmox` | Where the runtime should be scheduled. Docker and Kubernetes are GA; `proxmox` is known but release-blocked. `k3s` is accepted as a user-facing id and normalizes to `k8s` internally. |
...
| External runtime targets            | Same ingress as above                          | One Docker Compose host | Kubernetes (k3s/k8s) GA, NemoClaw experimental, Proxmox planned | Teams that need different runtime placement without changing operator workflow |
