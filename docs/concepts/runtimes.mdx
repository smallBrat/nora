...
The NemoClaw sandbox (`nemoclaw`) is experimental. It runs OpenClaw inside a secure, NVIDIA-powered sandbox environment with policy controls. It is built on [NVIDIA Nemotron](https://build.nvidia.com) models and uses OpenShell policy enforcement to restrict what the agent runtime can do inside the container.
