...
> Understand the runtimes Nora supports, including OpenClaw, and how to choose between the standard Docker sandbox and the NemoClaw secure sandbox.
...
<Note>
  OpenClaw is the most mature and best-supported runtime in Nora today. Nora is designed to support additional runtimes over time, but OpenClaw is the recommended path for first deployments.
</Note>
...
The NemoClaw sandbox (`nemoclaw`) runs OpenClaw inside a secure, NVIDIA-powered sandbox environment with policy controls. It is built on [NVIDIA Nemotron](https://build.nvidia.com) models and uses OpenShell policy enforcement to restrict what the agent runtime can do inside the container.
...
<Warning>
  NemoClaw must be enabled by your Nora operator before it is available as a sandbox option. If you select NemoClaw and it has not been enabled, deployment will fail with an error. Contact your operator to enable it.
</Warning>
...
|                   | Standard              | NemoClaw                                           |
| ----------------- | --------------------- | -------------------------------------------------- |
| **Availability**  | Always available      | Requires `nemoclaw` in `ENABLED_SANDBOX_PROFILES`  |
...
<Tip>
  If you are evaluating Nora for the first time, start with the standard sandbox. It requires no additional configuration and gives you the full OpenClaw experience through the Nora dashboard.
</Tip>
