...
<CardGroup cols={2}>
  <Card title="Twitter / X" href="/guides/integrations/twitter" icon="twitter" arrow>
    Click <b>Authorize with X</b>. Requires an X OAuth 2.0 app with the <code>tweet.read</code>, <code>tweet.write</code>, <code>users.read</code>, and <code>offline.access</code> scopes.
  </Card>
  <Card title="LinkedIn" href="/guides/integrations/linkedin" icon="linkedin" arrow>
    Click <b>Authorize with LinkedIn</b>. Requires a LinkedIn developer app with the <code>openid</code>, <code>profile</code>, <code>email</code>, and <code>w_member_social</code> scopes.
  </Card>
</CardGroup>
...
Prefer something else? Telegram, Slack, Discord, and email all work — see [Configure channels](/guides/channels) for each one's required fields. You only need one.
