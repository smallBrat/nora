...
<Step title="Set resource limits">
  Specify the vCPU count, RAM (MB), and disk size (GB) for the agent container. In self-hosted mode
  these values are clamped to the operator-configured limits. In PaaS mode they are fixed to your
  subscription plan.
</Step>
