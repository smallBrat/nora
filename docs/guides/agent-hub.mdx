...
- **Runtime configuration** — runtime family (OpenClaw or Hermes), deploy target (Docker, Kubernetes, Proxmox), sandbox profile, and the default container image.
...
<Card title="Set up Echo, the personal-branding agent" href="/guides/echo-personal-branding" icon="feather" arrow>
  A worked example: install the Echo listing, connect X and/or LinkedIn in the Integrations tab, connect a channel (WhatsApp) in the Channels tab, and run the bootstrap that learns your voice.
</Card>
