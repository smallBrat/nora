...
> Deploy agents with NVIDIA NemoClaw sandboxes for OpenShell policy controls, approval workflows, and Nemotron model support in Nora.
...
<Warning>
  NemoClaw must be enabled by your operator before you can use it. If `NEMOCLAW_ENABLED` is not set to `true` in the server environment, attempting to deploy a NemoClaw agent will return an error. Contact your Nora operator to enable it.
</Warning>
...
<Note>
  To use these models you must have an NVIDIA API key saved in **Settings** under the NVIDIA provider. See [Add and sync LLM provider keys](/guides/manage-providers).
</Note>
