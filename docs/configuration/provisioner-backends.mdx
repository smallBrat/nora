...
Nora provisions agent runtimes onto execution targets selected in the Deploy flow. Docker is the default path. Proxmox is enabled through `.env`, while Kubernetes clusters are registered in **Admin -> Kubernetes** so one Nora control plane can manage several clusters at once.
...
Use `ENABLED_BACKENDS=docker,proxmox` when you want both local Docker and Proxmox visible in the deploy flow. Do not add Kubernetes labels to `ENABLED_BACKENDS`; the Kubernetes deploy targets come only from the Admin cluster registry.
