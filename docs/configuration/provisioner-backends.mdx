...
Use `ENABLED_BACKENDS=docker,k3s` when you want both local Docker and K3s visible in the deploy flow. Use `ENABLED_BACKENDS=docker,k8s` for cloud Kubernetes. Set `K8S_PROVIDER=aks`, `gke`, `eks`, or `k3s` so the dashboard labels the target as the real connected backend, such as **Kubernetes on AKS**.

## Kubernetes providers

The Kubernetes adapter is provider-neutral. Nora talks to the Kubernetes API through `KUBECONFIG`, then creates Deployments and Services for each OpenClaw or Hermes agent. Cloud overlays label the shared `k8s` target as AKS, GKE, or EKS in the deploy flow.
