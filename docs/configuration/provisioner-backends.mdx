...
<CardGroup cols={3}>
  ...
  <Card title="Kubernetes" icon="cloud" href="/configuration/provisioner-backends/kubernetes">
    K3s, Kind, AKS, GKE, EKS, or another conformant Kubernetes cluster.
  </Card>
</CardGroup>
...
Use `ENABLED_BACKENDS=docker,k8s` when you want both local Docker and Kubernetes visible in the deploy flow. Use `ENABLED_BACKENDS=k8s` when every new agent should target Kubernetes by default.
...
<CardGroup cols={2}>
  ...
  <Card title="Kind" icon="flask" href="/configuration/provisioner-backends/kubernetes-kind">
    Local Kubernetes verification through `docker-compose.kind.yml`.
  </Card>
