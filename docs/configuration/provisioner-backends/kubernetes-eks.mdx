...
<Frame caption="AWS Console — EKS cluster overview">
  ![EKS cluster overview](/images/provisioner-backends/k8s/eks/cluster-overview.svg)
</Frame>
...
<Frame caption="AWS Console — Cluster → Compute (node group Ready)">
  ![EKS compute tab](/images/provisioner-backends/k8s/eks/compute-tab.svg)
</Frame>
...
<Frame caption="AWS Console — Cluster → Add-ons (AWS Load Balancer Controller / Auto Mode)">
  ![EKS add-ons](/images/provisioner-backends/k8s/eks/add-ons.svg)
</Frame>
...
```json theme={null}
{"service.beta.kubernetes.io/aws-load-balancer-scheme":"internet-facing","service.beta.kubernetes.io/aws-load-balancer-nlb-target-type":"ip"}
```
